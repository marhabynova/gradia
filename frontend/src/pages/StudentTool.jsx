  ...
  return (
    <div className="container" style={{ paddingTop: '6rem', paddingBottom: '6rem' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '4rem', animation: 'fade-in 1s ease' }}>
        <h1 className="text-gradient" style={{ fontSize: '3.5rem', marginBottom: '1rem' }}>AI Plagiarism Checker</h1>
        <p className="text-muted" style={{ fontSize: '1.25rem', maxWidth: '600px', margin: '0 auto' }}>
          Tingkatkan kualitas skripsimu secara instan. Lolos Turnitin, perbaiki format, dan basmi typo dengan kecerdasan buatan kelas Enterprise.
        </p>
      </div>
            ...
            >
              ...
              {file ? (
                <div style={{ animation: 'fade-in 0.3s ease' }}>
                  <h4 style={{ margin: '0 0 0.5rem 0', color: 'var(--success)', fontSize: '1.25rem' }}>File Terpilih Siap Dianalisis</h4>
                  <span style={{ color: 'var(--text-main)', fontWeight: 'bold', fontSize: '1.1rem' }}>{file.name}</span>
                </div>
              ) : (
                <span className="text-muted" style={{ fontSize: '1.1rem' }}>Klik atau seret file dokumen Anda ke area ini</span>
              )}
              <input 
                type="file" 
      ...
      {status === 'error' && (
        <div className="glass-panel glass-panel-hover" style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center', borderColor: 'rgba(239, 68, 68, 0.3)' }}>
          ...
          <h3 style={{ color: 'var(--danger)', fontSize: '1.5rem' }}>Upload Ditolak</h3>
      ...
      {status === 'uploading' && (
        <div className="flex-center" style={{ flexDirection: 'column', padding: '6rem 0' }}>
          ...
          <h3 style={{ color: isPremium ? '#f59e0b' : 'inherit', fontSize: '1.5rem', marginBottom: '0.5rem' }}>
            {isPremium ? 'Mesin Ghostwriter Aktif 👻' : 'Menganalisis Dokumen...'}
          </h3>
          <p className="text-muted" style={{ fontSize: '1.1rem' }}>
            {isPremium 
              ? 'Mencuci teks lintas bahasa & Menyuntikkan Sitasi Jurnal Asli...' 
              : 'AI Gemini sedang membaca struktur kalimatmu dan mendeteksi plagiarisme.'}
          </p>
        ...
        <div className="glass-panel glass-panel-hover" style={{ maxWidth: '800px', margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '3rem', borderBottom: '1px solid var(--glass-border)', paddingBottom: '2rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
              ...
              <div>
                <h2 style={{ margin: '0 0 0.5rem 0', fontSize: '1.75rem' }}>Analisis Selesai</h2>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  <span className="text-muted">Skor Keamanan Dokumen:</span>
                  <span style={{ background: 'rgba(239, 68, 68, 0.1)', color: 'var(--danger)', padding: '4px 12px', borderRadius: '99px', fontWeight: 'bold' }}>24% Plagiat</span>
                </div>
              </div>
            </div>
              ...
              <div style={{ display: 'flex', gap: '1rem' }}>
                {!paraphrasedText && !quotaError && (
                  <button className="btn-secondary" style={{ flex: 1, borderColor: 'rgba(255,255,255,0.1)' }} onClick={() => handleParaphrase('mock-chunk')} disabled={isParaphrasing}>
                    <Zap size={18} /> 
                    {isParaphrasing ? 'AI Sedang Bekerja...' : 'Ghostwriter AI (Parafrase)'}
                  </button>
                )}
                {!suggestedCitation && !quotaError && (
                  <button className="btn-primary" style={{ flex: 1, background: 'linear-gradient(135deg, #0ea5e9, #0284c7)' }} onClick={() => handleSuggestCitation('Metodologi penelitian ini...')} disabled={isSuggestingCitation}>
                    <FileText size={18} /> 
                    {isSuggestingCitation ? 'Mencari Referensi...' : 'Saran Sitasi Asli (Anti Plagiat)'}
                  </button>
                )}
              </div>
            </div>
      ...
      {showModal && !ticketData && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(8px)' }}>
          <div className="glass-panel" style={{ maxWidth: '550px', width: '90%', padding: '3rem', animation: 'fade-in 0.3s ease' }}>
            <h2 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>Pilih Jalur Unduhan</h2>
            <p className="text-muted" style={{ marginBottom: '2.5rem' }}>Dukung operasional server AI Gradia dengan memilih metode di bawah ini.</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              >
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontSize: '1.25rem', fontWeight: 'bold', marginBottom: '0.25rem' }}>Jalur Cepat (VIP Speed)</div>
                  <div style={{ fontSize: '0.9rem', opacity: 0.9, fontWeight: 'normal' }}>Buka sponsor kami 1x, langsung unduh detik ini juga.</div>
                </div>
              ...
              >
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontSize: '1.25rem', fontWeight: 'bold', color: 'var(--text-muted)', marginBottom: '0.25rem' }}>Jalur Biasa (Gratis)</div>
                  <div style={{ fontSize: '0.9rem', color: 'rgba(255,255,255,0.4)', fontWeight: 'normal' }}>Menunggu 60 detik antrean server di halaman ini.</div>
                </div>
            ...
            {countdown > 0 ? (
              <div style={{ margin: '3rem 0' }}>
                ...
                <div style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', padding: '1rem', borderRadius: 'var(--radius-md)' }}>
                  <p style={{ margin: 0, color: '#fca5a5', fontSize: '0.9rem' }}><strong>PERINGATAN:</strong> Jangan tutup atau <i>refresh</i> halaman ini, atau tiket akan hangus!</p>
                </div>
